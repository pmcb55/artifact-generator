...
import { RdfXmlParser as ParserRdfXml } from "rdfxml-streaming-parser";
import { RdfaParser as ParserRdfa } from "rdfa-streaming-parser";
...
const _debug = debug("artifact-generator:Resources");
...
export function clearResourceFromCache(resource: string) {
  const normalizedResource = Util.normalizePath(resource);
  _debug(`Clear resource [${normalizedResource}] from cache.`);
...
// failure.
export const DEFAULT_MODIFICATION_DATE = 662688059000;
...
export class Resource {
  ...
  termSelectionResource: string;
  vocabAcceptHeaderOverride: string;
  vocabContentTypeHeaderOverride: string;
  vocabContentTypeHeaderFallback: string;
  ...
  static async readResourceViaCache(
    ...
    if (cacheLookup) {
      _debug(`Loading resource from cache: [${inputResource}]`);
      return cacheLookup;
    }
  ...
  static readResource(
    ...
    _debug(`Loading resource from source: [${inputResource}]`);
    if (Resource.isOnline(inputResource)) {
      ...
      if (vocabAcceptHeaderOverride) {
        ...
        _debug(
          `Overriding Accept header with: [${vocabAcceptHeaderOverride}].`,
        );
      } else {
   ...
   * @returns {*}
   */
  static formatUrlWithFilenameCharacters(url: string): string {
   ...
   * @returns {*|string}
   */
  static addTurtleExtensionIfNeeded(resourceName: string): string {
   ...
   * @returns {string}
   */
  static quadToStringIgnoringBNodes(quad: Quad): string {
    return (
  ...
  static simpleStringHash(value: string): number {
    let hash = 0,
      i,
      chr;
  ...
  static loadTurtleFileIntoDatasetPromise(filename: string) {
  ...
  static async getHttpResourceLastModificationTime(
    resource: string,
  ): Promise<number> {
   ...
   * @param {*} resource
   */
  static async getResourceLastModificationTime(
    resource: string,
  ): Promise<number> {
