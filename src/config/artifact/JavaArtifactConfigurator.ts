...
import {
  ...
  ADD_REPOSITORY_CONFIRMATION,
} from "../ArtifactConfigurator.js";
...
export class JavaArtifactConfigurator extends ArtifactConfigurator {
  constructor() {
    super();
  ...
  static async promptMaven() {
    ...
    const mavenConfig: {
      ...
      groupId?: string;
      publish?: { key: string; command: string }[];
      packagingTemplates?: { fileName: string; template?: string }[];
      repository?: { id?: string; type?: string; url?: string }[];
    } = {
      packagingTool: "maven",
    };
