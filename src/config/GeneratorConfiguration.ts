...
const _debug = debug("artifact-generator:GeneratorConfiguration");

import { getArtifactDirectorySourceCode } from "../Util.js";

import { fileURLToPath } from "url";
const _dirname = path.dirname(fileURLToPath(import.meta.url));

export const DEFAULT_PUBLISH_KEY = "_default";

export const CONFIG_SOURCE_COMMAND_LINE = "<Command Line Config>";
...
export const DEFAULT_CLI_ARTIFACT = [
  {
...
export class GeneratorConfiguration {
   ...
   */
  static normalizeRelativePath(relativePath: string, configSource: string) {
    return path.join(
  ...
  static normalizeTemplatePath(
    ...
    if (templatePathInternal) {
      ...
      normalizedTemplate = GeneratorConfiguration.normalizeAbsolutePath(
        path.join(_dirname, RELATIVE_TEMPLATE_DIRECTORY, templatePathInternal),
        process.cwd(),
      );
   ...
   */
  static normalizeAbsolutePath(absolute: string, base: string) {
  ...
  static validateConfiguration(config, configSource) {
    ...
    config.vocabList.forEach((list, vocabIndex: number) => {
      ...
      list.inputResources.forEach((resource, inputIndex: number) => {
  ...
  static collectVocabFromCLI(args) {
    const vocab: { inputResources: string[]; termSelectionResource?: string } =
      { inputResources: [] };
   ...
   */
  async getInputResourcesChangedSince(timestamp: number) {
    const resources = [];
    for (let i = 0; i < this.configuration.vocabList.length; i += 1) {
