...
import { RDF, RDFS, SKOS, OWL } from "./CommonTerms.js";
import { Resource, DEFAULT_MODIFICATION_DATE } from "./Resource.js";
...
describe("Resources last modification", () => {
  ...
  it("should return default time for unreachable online resources", async () => {
    ...
    expect(lastModificationTime).toEqual(DEFAULT_MODIFICATION_DATE);
  });
});
    ...
    describe("storing local copy of vocab", () => {
      it("should store local copy", (done) => {
        ...
        const dataset = datasetFrom(vocabMetadata, [
          rdf.quad(OWL.Ontology, RDFS.subClassOf, SKOS.Concept),
        ]);
