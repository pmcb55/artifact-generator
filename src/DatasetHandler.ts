...
const debug = debugLib("artifact-generator:DatasetHandler");

import type { DatasetCore, Quad, Term, Literal } from "@rdfjs/types";
...
import { FileGenerator } from "./generator/FileGenerator.js";
import { BestPracticeReportGenerator } from "./generator/BestPracticeReportGenerator.js";
import { describeInput, describeTemplateVocab } from "./Util.js";

interface LiteralEntry {
  value: string;
  valueEscapedForJavaScript: string;
  valueEscapedForJava: string;
  language: string;
}

export interface TemplateInput {
  rollupVersion?: unknown;
  rollupBabelPluginVersion?: unknown;
  rollupCommonjsPluginVersion?: unknown;
  rollupNodeResolveVersion?: unknown;
  webpackVersion?: unknown;
  webpackCliVersion?: unknown;
  babelCoreVersion?: unknown;
  babelLoaderVersion?: unknown;
  generatedTimestamp?: unknown;
  generatorName?: unknown;
  artifactGeneratorVersion?: unknown;
  sourceRdfResources?: string;
  sourceTemplateVocab?: unknown;
  classes: unknown[];
  properties: unknown[];
  literals: unknown[];
  constantIris: unknown[];
  constantStrings: unknown[];
  inputResources?: unknown;
  vocabListFile?: unknown;
  vocabListFileIgnore?: unknown;
  vocabularyIri?: string;
  localNamespaceIri?: string;
  vocabName: string;
  vocabNameUpperCase?: string;
  namespaceIriOverride?: string;
  namespaceIri: string;
  ...
  solidCommonVocabVersion?: unknown;
  npmRegistry?: unknown;
  outputDirectory?: unknown;
  authorSet?: Set<string>;
  authorSetFormatted?: string;
  runNpmInstall?: unknown;
  runMavenInstall?: unknown;
  runNpmPublish?: unknown;
  runWidoco?: unknown;
  noPrompt?: unknown;
  supportBundling?: unknown;
  localCopyAsTurtleDirectory?: string;
...
export class DatasetHandler {
   ...
   */
  constructor(
    public fullDataset: DatasetCore<Quad>,
    public termSelectionDataset: DatasetCore<Quad>,
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    public vocabData: any,
  ) {
  ...
  termsProcessed: Map<string, null>;
   ...
   */
  handleTerm(quad: Quad, namespace: string, rdfType: Term) {
    const labels: LiteralEntry[] = [];
    ...
    for (const subQuad of this.termSelectionDataset.match(
      quad.subject,
    ...
    for (const subQuad of this.fullDataset.match(
      quad.subject,
      RDFS.label,
      null,
    )) {
    ...
    const definitions: LiteralEntry[] = [];
    ...
    if (
      ...
      if (skosMatches.size > 1) {
        throw new Error(
          `Vocabulary term [${fullName}] in ${DatasetHandler.describeNamespaceInUse(
            namespace,
            this.vocabData.namespaceIriOverride,
          )} - found [${skosMatches.size}] values for constant of type [${
            rdfType.value
    ...
    for (const subQuad of skosMatches) {
      DatasetHandler.add(definitions, subQuad);
    }
   ...
   */
  getSetOfPredicateObjects(
    subject: Term | null,
    predicate: Term,
    propertyNameInObject: string,
  ) {
    ...
    for (const subQuad of this.fullDataset.match(subject, predicate, null)) {
      valueSet.add(subQuad.object.value);
    }

    // Copy our result strings into a set of objects.
    let result: Set<Record<string, string>> | undefined;
    if (valueSet.size > 0) {
      result = new Set();
   ...
   * @returns {any}
   */
  static buildCompositeTermDescription(
    labels: LiteralEntry[],
    comments: LiteralEntry[],
    definitions: LiteralEntry[],
  ) {
    let termDescription: string | undefined = undefined;
  ...
  static describeMultipleLanguages(
    labels: LiteralEntry[],
    sortedLangTagsLabel: string | undefined,
    comments: LiteralEntry[],
    sortedLangTagsComment: string | undefined,
  ) {
    let termDescription: string | undefined = undefined;
   ...
   */
  static sortListOfLangTags(literals: LiteralEntry[]) {
  ...
  static filterOutEnglishAnNoLocale(literals: LiteralEntry[]) {
    return literals
      .filter(
  ...
  static countEnglishAnNoLocale(literals: LiteralEntry[]) {
    return literals.filter(
      (elem) => elem.language === "" || elem.language.startsWith("en"),
    ).length;
  }

  isValidIri(str: string) {
    try {
      new URL(str);
    } catch {
      return false;
  ...
  static add(array: LiteralEntry[], quad: Quad) {
    if (DatasetHandler.doesNotContainValueForLanguageAlready(array, quad)) {
      ...
      array.push({
        ...
        valueEscapedForJava: FileGenerator.escapeStringForJava(literal.value),
        language: literal.language,
      });
    }
  }

  static doesNotContainValueForLanguageAlready(
    array: LiteralEntry[],
    quad: Quad,
  ) {
    const literal = quad.object as Literal;
    return (
      array.length === 0 || !array.some((e) => e.language === literal.language)
    );
  }
  ...
  static lookupEnglishOrNoLanguage(collection: LiteralEntry[]) {
  ...
  handleClass(subject: string, result: TemplateInput) {
    const subjectNode = rdf.namedNode(subject);
    SUPPORTED_CLASS_TYPES.forEach((classType) => {
      for (const quad of this.fullDataset.match(
        subjectNode,
        RDF.type,
        classType,
      )) {
  ...
  handleProperty(subject: string, result: TemplateInput) {
    const subjectNode = rdf.namedNode(subject);
  ...
  handleLiteral(subject: string, result: TemplateInput) {
    const subjectNode = rdf.namedNode(subject);
  ...
  handleConstantString(subject: string, result: TemplateInput) {
    const subjectNode = rdf.namedNode(subject);
  ...
  handleConstantIri(subject: string, result: TemplateInput) {
    const subjectNode = rdf.namedNode(subject);
  ...
  static findLongestTermName(terms: string[]) {
   ...
   */
  static lookupKnownNamespacePrefix(namespaceIri: string) {
  ...
  static vocabNameUpperCase(name: string) {
    return name.toUpperCase().replace(/-/g, "_");
  }

  findDescription(vocabularyIri: string, descriptionFallback: string) {
  ...
  findAuthors(vocabularyIri: string) {
    ...
    return new Set(
      vocabAuthors.size === 0
        ? []
        : [...vocabAuthors].map((authorQuad) => authorQuad.object.value),
    );
  }
   ...
   * @returns {string|*}
   */
  lookupVocabularyIri(vocabularyIriOverride: string) {
    const allOwlOntologies = [
      ...this.fullDataset.match(null, RDF.type, OWL.Ontology),
    ];
  ...
  lookupNamespaceDetails(
    vocabularyIri: string | undefined,
    namespaceIriOverride: string,
    namespacePrefixOverride: string,
  ) {
    const result: {
      vannNamespaceIri?: string;
      vannNamespacePrefix?: string;
      shaclDeclareIri?: string;
      shaclNamespaceIri?: string;
      shaclNamespacePrefix?: string;
      heuristicNamespaceIri?: string;
      detectedNamespaceIri?: string;
      namespaceIri?: string;
  ...
  lookupOneAndOnlyOnePredicate(
    vocabularyIri: string | undefined,
    predicateValueOverride: string | undefined,
    predicate: Term,
  ) {
    if (!vocabularyIri) {
      return undefined;
    ...
    const allPredicateQuads = [
      ...this.fullDataset.match(rdf.namedNode(vocabularyIri), predicate, null),
    ];
  ...
  static subjectsOnly(dataset: DatasetCore<Quad>) {
    const terms = dataset;
  ...
  static firstDatasetValue(
    ...
    if (languageTag !== undefined) {
      ...
      result = quads.find((elem) =>
        (elem.object as Literal).language.startsWith(languageTag),
      );
      if (result) {
      ...
      result = quads.find((elem) =>
        XSD.string.equals((elem.object as Literal).datatype),
      );
      if (result) {
        return result.object.value;
      }
