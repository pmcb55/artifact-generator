...
import { datasetFrom } from "./Util.js";
...
const vocabMetadata = datasetFrom([
...
describe("Dataset Handler", () => {
  describe("Handle multiple owl:Ontology triples", () => {
    it("should bomb out if multiple owl:Ontology triples", async () => {
      ...
      const datasetMultipleOntologies = datasetFrom(vocabMetadata, [
        rdf.quad(NAMESPACE_IRI, RDFS.label, DEFAULT_DESCRIPTION),
        rdf.quad(NAMESPACE_IRI_2, RDF.type, OWL.Ontology),
        rdf.quad(NAMESPACE_IRI_2, RDFS.label, DEFAULT_DESCRIPTION),
      ]);
    ...
    it("should warn if multiple owl:Ontology triples, but namespace overridden", async () => {
      ...
      const datasetMultipleOntologies = datasetFrom(vocabMetadata, [
        ...
        rdf.quad(NAMESPACE_IRI_2, RDF.type, OWL.Ontology),
        rdf.quad(NAMESPACE_IRI_2, RDFS.label, DEFAULT_DESCRIPTION),
        rdf.quad(NAMESPACE_IRI_OVERRIDE, RDFS.label, DEFAULT_DESCRIPTION),
      ]);
  ...
  describe("Ontology description ", () => {
    it("should pick up alternative vocab description predicates", async () => {
      const datasetRdfsLabel = datasetFrom(vocabMetadata, [
        rdf.quad(NAMESPACE_IRI, RDFS.label, DEFAULT_DESCRIPTION),
        rdf.quad(NAMESPACE_TEST_TERM, RDF.type, RDFS.Class),
      ]);
    ...
    it("should pick up vocab description when non-English", async () => {
      ...
      const datasetRdfsLabel = datasetFrom(vocabMetadata, [
        rdf.quad(NAMESPACE_IRI, RDFS.label, description),
        rdf.quad(NAMESPACE_TEST_TERM, RDF.type, RDFS.Class),
      ]);
    ...
    it("should use English description if multiple", async () => {
      ...
      const dataset = datasetFrom(vocabMetadata, [
        rdf.quad(NAMESPACE_IRI, RDFS.comment, commentInIrish),
        rdf.quad(NAMESPACE_IRI, RDFS.comment, commentInFrench),
        rdf.quad(NAMESPACE_IRI, RDFS.comment, commentInEnglish),
        rdf.quad(NAMESPACE_TEST_TERM, RDF.type, RDFS.Class),
      ]);
    ...
    it("should use description that starts with explicit English tag", async () => {
      ...
      const dataset = datasetFrom(vocabMetadata, [
        rdf.quad(NAMESPACE_IRI, RDFS.comment, commentInIrish),
        rdf.quad(NAMESPACE_IRI, RDFS.comment, commentInFrench),
        rdf.quad(NAMESPACE_IRI, RDFS.comment, commentInUsEnglish),
        rdf.quad(NAMESPACE_TEST_TERM, RDF.type, RDFS.Class),
      ]);
    ...
    it("should fallback to no language tag if no explicit English", async () => {
      ...
      const dataset = datasetFrom(vocabMetadata, [
        rdf.quad(NAMESPACE_IRI, RDFS.comment, commentInIrish),
        rdf.quad(NAMESPACE_IRI, RDFS.comment, commentInFrench),
        rdf.quad(NAMESPACE_IRI, RDFS.comment, commentWithNoLocale),
        rdf.quad(NAMESPACE_TEST_TERM, RDF.type, RDFS.Class),
      ]);
  ...
  describe("handling multilingual descriptions for labels, comments, etc.", () => {
    ...
    it("should give full description of matching labels and comments in all languages", async () => {
      const dataset = datasetFrom(vocabMetadata, [
        rdf.quad(NAMESPACE_IRI, RDFS.label, DEFAULT_DESCRIPTION),
        rdf.quad(NAMESPACE_TEST_TERM, RDF.type, RDFS.Class),
        rdf.quad(NAMESPACE_TEST_TERM, RDFS.label, labelInIrish),
        rdf.quad(NAMESPACE_TEST_TERM, RDFS.comment, commentInIrish),
        rdf.quad(NAMESPACE_TEST_TERM, RDFS.label, labelInFrench),
        rdf.quad(NAMESPACE_TEST_TERM, RDFS.comment, commentInFrench),
        rdf.quad(NAMESPACE_TEST_TERM, RDFS.label, labelInEnglish),
        rdf.quad(NAMESPACE_TEST_TERM, RDFS.comment, commentInEnglish),
        rdf.quad(NAMESPACE_TEST_TERM, RDFS.label, labelNoLocale),
        rdf.quad(NAMESPACE_TEST_TERM, RDFS.comment, commentNoLocale),
      ]);
    ...
    it("should treat 'skosxl:literalForm' and 'skos:prefLabel' as labels in all languages", async () => {
      const dataset = datasetFrom(vocabMetadata, [
        rdf.quad(NAMESPACE_IRI, RDFS.label, DEFAULT_DESCRIPTION),
        rdf.quad(NAMESPACE_TEST_TERM, RDF.type, SKOSXL.Label),
        rdf.quad(NAMESPACE_TEST_TERM, SKOSXL.literalForm, labelInIrish),
        rdf.quad(NAMESPACE_TEST_TERM, SKOS.prefLabel, labelInFrench),
        rdf.quad(NAMESPACE_TEST_TERM, SKOSXL.literalForm, labelInEnglish),
        rdf.quad(NAMESPACE_TEST_TERM, SKOS.prefLabel, labelNoLocale),
      ]);
    ...
    it("should say no long-form description if no comment at all", async () => {
      const dataset = datasetFrom(vocabMetadata, [
        ...
        rdf.quad(NAMESPACE_TEST_TERM, RDFS.label, rdf.literal("label no lang")),
      ]);
  ...
  describe("handle sub-classes or sub-properties", () => {
    it("should handle sub-classes", async () => {
      const dataset = datasetFrom(vocabMetadata, [
        rdf.quad(NAMESPACE_IRI, RDFS.label, DEFAULT_DESCRIPTION),
        rdf.quad(NAMESPACE_TEST_TERM, RDFS.subClassOf, SKOS.Concept),
      ]);
  ...
  it("should makes exceptions for vocab terms found in common vocabs - RDF:langString", async () => {
    const dataset = datasetFrom(vocabMetadata, [
      rdf.quad(NAMESPACE_IRI, DCTERMS.description, DEFAULT_DESCRIPTION),
      rdf.quad(RDF.langString, RDF.type, RDFS.Datatype),
    ]);
  ...
  describe("Best Practice compliance report", () => {
    it("should generate Best Practice compliance report if configured", async () => {
      const dataset = datasetFrom(vocabMetadata, [
        rdf.quad(NAMESPACE_IRI, DCTERMS.description, DEFAULT_DESCRIPTION),
        rdf.quad(rdf.namedNode(`${NAMESPACE}testTerm`), RDF.type, RDF.Property),
      ]);
