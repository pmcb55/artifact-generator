...
import { DEFAULT_PUBLISH_KEY } from "../config/GeneratorConfiguration.js";

const _debug = debug("artifact-generator:ArtifactGenerator");
...
export class ArtifactGenerator {
  ...
  async generatePackaging() {
    ...
    if (this.areWeConfiguredToGenerateResources()) {
      ...
      this.artifactData.artifactToGenerate.forEach((artifactInfo) => {
      ...
      FileGenerator.createFileFromTemplate(
        `${_dirname}/../../template/README-artifactBundle.hbs`,
      ...
      if (publishConfigs) {
        for (let j = 0; j < publishConfigs.length; j += 1) {
          ...
          if (
            ...
            _debug(`Changing to directory [${runFrom}].`);
            ...
            try {
              publishConfigs[j].command
                ...
                .map((str) => str.trim())
                .map((command) => {
                  _debug(`Running sub-command [${command}]...`);
                  let response;
                  try {
                  ...
                  _debug(
                    `Response from running [${command}] was [${response}]`,
                  );
                });
            } finally {
