...
import {
  ...
  LANGUAGE as JAVA,
} from "../config/artifact/JavaArtifactConfigurator.js";

import {
  NodeArtifactConfigurator,
  LANGUAGE as JAVASCRIPT,
} from "../config/artifact/NodeArtifactConfigurator.js";

import { ArtifactConfigurator } from "../config/ArtifactConfigurator.js";
import { VocabularyConfigurator } from "../config/VocabularyConfigurator.js";
...
const SUPPORTED_LANGUAGES: Record<string, new () => ArtifactConfigurator> = {};
...
export function validateLanguageCheckboxes(answer: string[]): true | string {
  if (answer.length < 1) {
...
export function validateRepositoryCheckboxes(answer: string[]): true | string {
  if (answer.length > 1) {
    // This mismatch in return types is expected by inquirer
...
export class ConfigFileGenerator {
  ...
  static buildConfigGenerator(language) {
    ...
    throw new Error(
      `Unsupported language: no config generator is registered for [${language}]`,
    );
  }
