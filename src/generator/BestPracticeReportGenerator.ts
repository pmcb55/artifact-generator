...
import { curie } from "../Util.js";
// eslint-disable-next-line @typescript-eslint/no-unused-vars
import { INRUPT_BEST_PRACTICE_NAMESPACE } from "../CommonTerms.js";

type IsDefinedByEntry = { isDefinedBy: string };

interface ComplianceReportState {
  totalTermCount: number;
  termsWithIsDefinedBy: unknown[];
  bpReport_0?: string;
  bpReport_1?: string;
  bpReport_2?: string;
...
const _debug = debug("artifact-generator:BestPracticeReportGenerator");
...
export class BestPracticeReportGenerator {
  static get DISPLAY_TERM_LIMIT() {
    return 5;
  }

  static buildComplianceReport(vocabInfo) {
    const reportSoFar: ComplianceReportState = {
      totalTermCount: 0,
      termsWithIsDefinedBy: [],
    };

    _debug("Generating Best Practice Report");
  ...
  static complianceBp_namespaceIriProvidedExplicitly(vocabInfo, reportSoFar) {
    // Previously used but currently commented out:
    // const languages = ["en"];
  ...
  static isDefinedByIri(termsWithIsDefinedBy, iri: string) {
    return termsWithIsDefinedBy.filter((term) =>
      Array.from(term.isDefinedBys).find(
        (definedBy: IsDefinedByEntry) => definedBy.isDefinedBy === iri,
      ),
    );
  }

  static isDefinedByOtherIri(termsWithIsDefinedBy, ignoreIris: string[]) {
    ...
    const termsDefinedByOtherIri = termsWithIsDefinedBy.filter((term) => {
      const otherIri = Array.from(term.isDefinedBys).filter(
        (termIri: IsDefinedByEntry) =>
          !ignoreIris.includes(termIri.isDefinedBy),
      );
      if (otherIri.length > 0) {
        otherIri.forEach((match: IsDefinedByEntry) =>
          otherIris.add(match.isDefinedBy),
        );
      }
