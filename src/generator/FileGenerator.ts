...
const _debug = debug("artifact-generator:FileGenerator");
...
export class FileGenerator {
  ...
  static createSharedPackagedFiles(generalInfo, artifactInfo) {
    ...
    FileGenerator.createFileFromTemplate(
      `${_dirname}/../../template/README-package.hbs`,
   ...
   */
  static escapeStringForJson(value: string): string {
    // Just use JSON.stringify, but make sure we strip off the enclosing quotes!
    const escaped = JSON.stringify(value);
    return escaped.substr(1, escaped.length - 2);
  }

  static escapeStringForJavaScript(value: string): string {
    return value.replace(/`/g, "\\`");
  }

  static escapeStringForJava(value: string): string {
    return value
      .replace(/\\/g, "\\\\\\\\")
      .replace(/"/g, '\\"')
      .replace(
