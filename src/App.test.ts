...
} from "./generator/ArtifactGenerator.js";
import {
  ConfigFileGenerator,
  DEFAULT_CONFIG_TEMPLATE_PATH,
} from "./generator/ConfigFileGenerator.js";
import { VocabWatcher } from "./VocabWatcher.js";

jest.mock("./generator/ArtifactGenerator", () => ({
  ArtifactGenerator: jest.fn(),
}));
jest.mock("./VocabWatcher", () => ({
  VocabWatcher: jest.fn(),
}));
jest.mock("./generator/ConfigFileGenerator", () => {
  const path = require("path");
  return {
    ConfigFileGenerator: jest.fn(),
    DEFAULT_CONFIG_TEMPLATE_PATH: path.join(
      process.cwd(),
      "template",
      "initial-config.hbs",
    ),
...
(ArtifactGenerator as unknown as jest.Mock).mockImplementation(
  nonPublishingGenerator,
);

(VocabWatcher as unknown as jest.Mock).mockImplementation(() => {
  return {
    watch: jest.fn((x) => x),
    unwatch: jest.fn((x) => x),
  };
...
(ConfigFileGenerator as unknown as jest.Mock).mockImplementation(() => {
  return {
    ...
    generateConfigFile: (targetPath: string) => {
      FileGenerator.createFileFromTemplate(
        DEFAULT_CONFIG_TEMPLATE_PATH,
        // Provide variables expected by template(s).
        {
...
describe("App tests", () => {
  describe("Testing mocked generator...", () => {
    it("should pass through in non-quiet mode (with DEBUG setting too)", async () => {
      debugLib.enable("artifact-generator:*");
